...
<body>
  ...
  <div class="main">
	...
	<form method="post" action="" enctype="multipart/form-data">
	  <input style="width:250px;" type="text" name="user_query" placeholder="Search a Product" />
	...
	<?php 
	  ...
	  if(empty($_POST['user_query'])) {
	    $location = $_GET['location'];
	    echo "<script>alert('Empty search..Please try again..')</script>";
	    echo "<script>window.location.replace('all_products.php?location=$location');</script>";
	  } else {
	    $location = $_GET['location'];
	    echo "<script>window.location.replace('all_products.php?location=$location&user_query=$user_query&search=$search');</script>";
  	  }
  	} 
	...
	<div class="navigation">
	  ...
	  <a id="myBtnLoc">Explore Live Product Offers</a>

	<?php if(isset($_SESSION['user_id'])) { ?>
	  <a href="my_account.php?action=account">My Account</a>
	<?php } ?>
	...
	<!--- The User Location Modal --->
  	<div id="myModalLoc" class="modalLoc">

    	<!--- Modal content --->
    	  <div class="modal-contentLoc">
    	    <span class="closeLoc">&times;</span>
	...
	if(isset($_POST['submit_location'])) {
	  ...
	  if(in_array($location_value, $london_locations)) {
 	  ...
 	  <script>
	    var modal = document.getElementById("myModalLoc");
  ...
  <script>
    ...
    var btnLoc = document.getElementById("myBtnLoc");
