<?php 
  if(!empty($_GET['tx'])){
	...
	$insert_delivery = mysqli_query($con, "insert into FYP_Delivery (invoice_id, guest_email, name, address, city, postcode, country, contact, add_info, delivery_type)
	 values ('$fetch_cart[invoice_number]', '$fetch_customer[guest_email]', '$fetch_customer[guest_name]', '$fetch_customer[guest_address]', '$fetch_customer[guest_city]', '$fetch_customer[guest_postcode]', '$fetch_customer[guest_country]', '$fetch_customer[guest_contact]', '$add_info', '$fetch_cart[delivery_time]')");
...
<?php
  ...
  $to = "michbodzio97@yahoo.com"; //$fetch_payment['guest_email'];
  $subject = "Guest Purchase Order Details | W171216 Online Shopping";
